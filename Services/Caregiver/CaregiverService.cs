...
public sealed class CaregiverService : ICaregiverService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (link.Patient?.UserId != null)
        {
            await _notificationService.SendNotificationAsync(
                link.Patient.UserId,
                "Link Approved",
                $"{caregiver.User.FullName} has approved your caregiver link request.",
                "System",
                "/patient/dashboard",
                cancellationToken);
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (link.Patient?.UserId != null)
        {
            await _notificationService.SendNotificationAsync(
                link.Patient.UserId,
                "Link Rejected",
                $"{caregiver.User.FullName} has rejected your caregiver link request.",
                "System",
                "/patient/dashboard",
                cancellationToken);
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var revokerIdStr = requestingUserId.ToString();
        if (link.Patient?.UserId == revokerIdStr && link.Caregiver?.UserId != null)
        {
            await _notificationService.SendNotificationAsync(
                link.Caregiver.UserId,
                "Link Revoked",
                $"Your access to {link.Patient.User?.FullName ?? "the patient"}'s profile has been revoked.",
                "System",
                "/caregiver/dashboard",
                cancellationToken);
        }
        else if (link.Caregiver?.UserId == revokerIdStr && link.Patient?.UserId != null)
        {
            await _notificationService.SendNotificationAsync(
                link.Patient.UserId,
                "Link Revoked",
                $"{link.Caregiver.User?.FullName ?? "The caregiver"} is no longer your caregiver.",
                "System",
                "/patient/dashboard",
                cancellationToken);
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var upcomingAppointments = await _db.Consultations
            .Include(c => c.Doctor)
                .ThenInclude(d => d.User)
            .AsNoTracking()
            .Where(c => c.PatientId == patientId && c.Status == "Scheduled" && c.ScheduledDateTime >= DateTime.UtcNow)
            .OrderBy(c => c.ScheduledDateTime)
            .Take(5)
            .Select(c => new CaregiverAppointmentResponse
            {
                AppointmentId = c.ConsultationId,
                DoctorFullName = c.Doctor.User.FullName,

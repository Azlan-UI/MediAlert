...
// ─── 2. DATABASE — Entity Framework Core + SQL Server ────────────────────────
// UseSqlServer is the SQL Server driver for EF Core.
...
builder.Services.AddAuthorization(options =>
{
    ...
    options.AddPolicy(AuthorizationPolicies.PremiumDoctor, policy =>
    {
...
builder.Services.AddScoped<MediAlert.Services.Notifications.INotificationService, MediAlert.Services.Notifications.NotificationService>();
builder.Services.AddSingleton<Microsoft.AspNetCore.SignalR.IUserIdProvider, MediAlert.Hubs.CustomUserIdProvider>();
builder.Services.AddSingleton<MediAlert.Services.Notifications.NotificationNotifier>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        // Ensure that if chaudharyhasnat786@gmail.com exists as a Doctor user, they have a record in the Doctors table
        var doctorUser = await userManager.FindByEmailAsync("chaudharyhasnat786@gmail.com");
        if (doctorUser != null)
        {
            var hasDoctorProfile = await dbContext.Doctors.AnyAsync(d => d.UserId == doctorUser.Id);
            if (!hasDoctorProfile)
            {
                logger.LogInformation("Doctor profile missing for chaudharyhasnat786@gmail.com. Creating it now...");
                dbContext.Doctors.Add(new Doctor
                {
                    DoctorId = Guid.NewGuid(),
                    UserId = doctorUser.Id,
                    LicenseNumber = "LIC-DEFAULT-786",
                ...
                logger.LogInformation("Doctor profile created successfully.");
            }
        }

        var patientsList = await dbContext.Patients.Include(p => p.User).ToListAsync();
        foreach (var p in patientsList)
        {
            var subs = await dbContext.Subscriptions.Where(s => s.PatientId == p.PatientId).ToListAsync();
            logger.LogInformation(">>> DATABASE PATIENT: Email={Email}, Subscriptions={Subs}", 
                p?.User?.Email ?? "NoEmail", 
                subs.Any() ? string.Join(", ", subs.Select(s => $"{s.Tier}:{s.Status}")) : "None");
        }
    }
    catch (Exception ex)
    {
...
app.MapHub<MediAlert.Hubs.NotificationHub>("/hubs/notifications");

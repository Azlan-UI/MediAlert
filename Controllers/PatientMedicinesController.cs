...
public class PatientMedicinesController : ControllerBase
{
    ...
    public PatientMedicinesController(
        ...
        MediAlert.Services.Notifications.INotificationService notificationService)
    {
    ...
    public async Task<IActionResult> GetMedicines(CancellationToken cancellationToken, [FromQuery] Guid? patientId = null)
    {
        Guid targetPatientId;
        
        if (User.IsInRole(UserRoles.Doctor))
        {
            if (patientId == null) return BadRequest(new PatientMedicationErrorResponse { Message = "patientId is required for doctors." });
            
            var doctorUserId = GetUserId();
            var doctor = await _dbContext.Doctors.FirstOrDefaultAsync(d => d.UserId == doctorUserId.ToString(), cancellationToken);
            if (doctor == null) return Unauthorized();
            
            var hasLink = await _dbContext.PatientDoctorLinks.AnyAsync(l => l.PatientId == patientId && l.DoctorId == doctor.DoctorId, cancellationToken);
        ...
        var medicines = await LoadMedicineResponsesAsync(targetPatientId, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Notify caregivers
        var caregiverUserIds = await _dbContext.CaregiverPatientLinks
            .Include(l => l.Caregiver)
            .Where(l => l.PatientId == patient.PatientId && l.Status == LinkStatuses.Approved)
        ...
        foreach (var caregiverUserId in caregiverUserIds)
        {
            if (!string.IsNullOrEmpty(caregiverUserId))
            {
                await _notificationService.SendNotificationAsync(
                    caregiverUserId,
                    "Medication Added",
                    $"{patient.User?.FullName ?? "Your patient"} added a new medication: {medication.DrugName}.",
                    "Medication",
                    "/caregiver/dashboard",
                    cancellationToken);
            }
            ...
            foreach (var caregiverUserId in caregiverUserIds)
            {
                if (!string.IsNullOrEmpty(caregiverUserId))
                {
                    await _notificationService.SendNotificationAsync(
                        caregiverUserId,
                        "Medication Updated",
                        $"{patient.User?.FullName ?? "Your patient"} updated medication: {medication.DrugName}.",
                        "Medication",
                        "/caregiver/dashboard",
                        cancellationToken);
                }
            ...
            foreach (var caregiverUserId in caregiverUserIds)
            {
                if (!string.IsNullOrEmpty(caregiverUserId))
                {
                    await _notificationService.SendNotificationAsync(
                        caregiverUserId,
                        "Medication Removed",
                        $"{patient.User?.FullName ?? "Your patient"} stopped/removed medication: {medication.DrugName}.",
                        "Medication",
                        "/caregiver/dashboard",
                        cancellationToken);
                }
